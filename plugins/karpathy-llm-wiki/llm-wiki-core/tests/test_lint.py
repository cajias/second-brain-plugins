...
# ---------------------------------------------------------------------------
# Single-file lint (--file)
# ---------------------------------------------------------------------------


class TestLintSingleFile:
    """``kb lint --file`` fast-path with tiered exit codes."""

    def test_clean_file_exits_zero(self, populated_wiki: Path, monkeypatch):
        monkeypatch.chdir(populated_wiki)
        note = populated_wiki / "wiki" / "permanent" / "api-gateway-auth-pattern.md"
        result = runner.invoke(app, ["lint", "--file", str(note)])
        assert result.exit_code == 0

    def test_missing_fields_exits_two(self, wiki_root: Path, sample_note_missing_fields: str, monkeypatch):
        permanent = wiki_root / "wiki" / "permanent"
        note = permanent / "incomplete.md"
        note.write_text(sample_note_missing_fields)
        monkeypatch.chdir(wiki_root)
        result = runner.invoke(app, ["lint", "--file", str(note)])
        assert result.exit_code == 2

    def test_rogue_tags_exits_two(self, wiki_root: Path, sample_note_rogue_tags: str, monkeypatch):
        permanent = wiki_root / "wiki" / "permanent"
        note = permanent / "rogue.md"
        note.write_text(sample_note_rogue_tags)
        monkeypatch.chdir(wiki_root)
        result = runner.invoke(app, ["lint", "--file", str(note)])
        assert result.exit_code == 2

    def test_broken_wikilink_soft_warning(self, wiki_root: Path, monkeypatch):
        permanent = wiki_root / "wiki" / "permanent"
        note = permanent / "has-broken-link.md"
        note.write_text("""\
---
id: perm-20260420-brk01
type: permanent
knowledge_type: fact
status: approved
confidence: high
...
""")
    ...
    def test_orphan_soft_warning(self, populated_wiki: Path, monkeypatch):
        monkeypatch.chdir(populated_wiki)
        note = populated_wiki / "wiki" / "permanent" / "orphan-note.md"
        result = runner.invoke(app, ["lint", "--file", str(note)])
        assert result.exit_code == 0

    def test_non_permanent_file_skipped(self, wiki_root: Path, monkeypatch):
        outside = wiki_root / "raw" / "inbox" / "some-raw.md"
        outside.write_text("# Not a permanent note\n")
        monkeypatch.chdir(wiki_root)
        result = runner.invoke(app, ["lint", "--file", str(outside)])
        assert result.exit_code == 0
